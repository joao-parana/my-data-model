...
class PostgresSchemaExtractor:
    ...
    def _get_referenced_by(self, schema: str, table_name: str) -> Dict[str, ReferencedBy]:
        query = text("""
            SELECT
            ...
            AND cl.relname = :table_name
        """)
        result = self.session.execute(query, {"schema": schema, "table_name": table_name})
        return {
            row[0]: ReferencedBy(
                table=row[0],
                columns=[row[1]],
    ...
    def extract_schema(self) -> DatabaseModel:
        ...
        schemas = [s for s in self.inspector.get_schema_names() if s not in ('information_schema', 'pg_catalog')]
        ...
        for schema_name in schemas:
            ...
            for table_name in tables:
                ...
                # Referenced By
                table_model.referenced_by = self._get_referenced_by(schema_name, table_name)
                
                schema_model.tables[table_name] = table_model
            
            database_model.schemas[schema_name] = schema_model
            print(f"schema_model = {schema_model}")    
